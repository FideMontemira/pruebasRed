...
// El mock del modelo debe ir antes de que se carguen los controladores
...
beforeAll(() => {
    ...
    app = server.app; // usamos la instancia de express
});
...
describe('Rutas /api/usuarios - pruebas de integración', () => {
    ...
    test('POST /api/usuarios debe devolver 400 si faltan campos', async () => {
        const res = await request(app)
            .post('/api/usuarios')
            .send({ email: 'juan@test.com' }); // faltan campos
